...
const SCENARIOS = [
  ...
  {
    name: 'bookmark-conflict',
    setupOrigin: true,
    steps: [
      '#jj commit -m "A: base"', '#jj bookmark create main -r @-', '#jj git push --bookmark main',
  ...
  },
  {
    ...
    setupOrigin: true,
    steps: [
      '#jj describe -m "fix: retry logic"', 'sh: echo v1 > retry.js',
      '#jj bookmark create feat', '#jj git push --bookmark feat',
      { in: '.', run: 'sh: git clone -q origin.git colleague' },
      'sh: echo v2 > retry.js',
      { in: 'colleague', run: 'sh: git checkout -q feat && echo maintainer > retry.js && git add -A && git commit -q --amend --no-edit && git push -qf origin feat' },
      'jj git fetch',
      'jj log',
      'jj bookmark list',
      'jj abandon @',
